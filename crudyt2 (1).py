...
Original file is located at
    https://colab.research.google.com/drive/1Dx5vTA7C-pTSUK89xa60x47lwnRtL27h
"""
...
def main():
    ...
    with tab1:  # Upload Video
        ...
        video_file = st.file_uploader("Choose a video file", type=["mp4"], key="upload_video_file")
        title = st.text_input("Title", key="upload_title")
        description = st.text_area("Description", key="upload_description")
        category_id = st.text_input("Category ID", key="upload_category_id")
    ...
    with tab2:  # Get Video Details
        ...
        video_id = st.text_input("Enter Video ID", key="read_video_id")

        if st.button("Get Details", key="read_button"):
            if video_id:
                youtube = authenticate()
    ...
    with tab3:  # Update Video Details
        ...
        video_id = st.text_input("Enter Video ID", key="update_video_id")
        new_title = st.text_input("New Title", key="update_new_title")
        new_description = st.text_area("New Description", key="update_new_description")
        new_tags = st.text_input("New Tags (comma-separated)", key="update_new_tags")
    ...
    with tab4:  # Delete Video
        ...
        video_id = st.text_input("Enter Video ID", key="delete_video_id")
